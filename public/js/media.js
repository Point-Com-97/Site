function charger_medias(critere, csrf) {
    fetch(`/admin/media/endpoint/sort.php?tri=${critere}&csrf_token=${csrf}`)
...
document.querySelectorAll('.item-media').forEach(function(lien) {
    lien.addEventListener('click', function(event) {
        ...
        charger_medias(critere, this.dataset.csrf);
    });
});

function remove_media(id, csrf) {
    if (!confirm('Supprimer ce média définitivement ?')) {
        return;
    }

    fetch(`/admin/media/endpoint/delete.php?id=${id}&csrf_token=${csrf}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
